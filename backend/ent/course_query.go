...
func (cq *CourseQuery) QueryTrainings() *TrainingQuery {
	query := &TrainingQuery{config: cq.config}
...
func (cq *CourseQuery) QueryDetails() *DetailQuery {
	query := &DetailQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CourseQuery) WithTrainings(opts ...func(*TrainingQuery)) *CourseQuery {
	query := &TrainingQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withTrainings = query
	return cq
}
...
func (cq *CourseQuery) WithDetails(opts ...func(*DetailQuery)) *CourseQuery {
	query := &DetailQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withDetails = query
	return cq
}
...
//
//	client.Course.Query().
//		GroupBy(course.FieldNamecourse).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (cq *CourseQuery) sqlAll(ctx context.Context) ([]*Course, error) {
	...
	if query := cq.withTrainings; query != nil {
		...
		query.Where(predicate.Training(func(s *sql.Selector) {
			s.Where(sql.InValues(course.TrainingsColumn, fks...))
		}))
	...
	if query := cq.withDetails; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Course)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Detail(func(s *sql.Selector) {
			s.Where(sql.InValues(course.DetailsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
