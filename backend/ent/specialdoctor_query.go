...
func (sq *SpecialdoctorQuery) QuerySpecialist() *SpecialistQuery {
	query := &SpecialistQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SpecialdoctorQuery) WithSpecialist(opts ...func(*SpecialistQuery)) *SpecialdoctorQuery {
	query := &SpecialistQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withSpecialist = query
	return sq
}
...
func (sq *SpecialdoctorQuery) sqlAll(ctx context.Context) ([]*Specialdoctor, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			sq.withSpecialist != nil,
			sq.withOffices != nil,
		}
	)
	if sq.withDepartment != nil || sq.withSpecialist != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withDoctor; query != nil {
		...
		query.Where(predicate.Doctor(func(s *sql.Selector) {
			s.Where(sql.InValues(specialdoctor.DoctorColumn, fks...))
		}))
	...
	if query := sq.withSpecialist; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Specialdoctor)
		for i := range nodes {
